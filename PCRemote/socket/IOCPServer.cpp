...
// 	}
	...
	WSAIoctl(pContext->m_Socket, SIO_KEEPALIVE_VALS, &klive,sizeof(tcp_keepalive),NULL,0, (unsigned long *)&chOpt,0,NULL);
...
unsigned CIOCPServer::ThreadPoolFunc (LPVOID thisContext)    
{
	...
	for (BOOL bStayInPool = TRUE; bStayInPool && pThis->m_bTimeToKill == false; ) 
	{
		...
		BOOL bIORet = GetQueuedCompletionStatus( hCompletionPort, &dwIoSize,
			             (PULONG_PTR) &lpClientContext, &lpOverlapped, INFINITE);
		...
		{			
			...
			if (nBusyThreads == pThis->m_nCurrentThreads)
			{
				if (nBusyThreads < pThis->m_nThreadPoolMax)
				{
					...
					{
						UINT nThreadID = -1;
//						HANDLE hThread = (HANDLE)_beginthreadex(NULL, 0,ThreadPoolFunc,(void*) pThis,0, &nThreadID);
//						CloseHandle(hThread);
					}
				}
			}
